  ...
  return async (request, response) => {
    try {
      if (request.method === 'GET') {
        ...
        try {
          const value = await previewResponse(ctx, engine, coordinator, sessionId, messageSeq, detailsOnly, offset, limit)
          json(response, 200, value)
        } catch (error) {
          if (isNotGitRepository(error)) {
            json(response, 200, { status: 'missing', reason: 'not-a-git-repository', fileRestoreAvailable: false })
            return
          }
          throw error
        }
        ...
        if (action === 'rewindConversation') {
          ...
          if (!archived.result.ok) {
            throw new ChangeLedgerError(
              ...
              `the conversation-only revision was created, but the previous revision could not be archived: ${archived.result.error.message}`,
            )
          }
...
/** Build the file-restore preview for one message, throwing only on genuine errors. */
async function previewResponse(
  ctx: Pick<Context, 'sessions' | 'sessionQuery'> & { readonly agents?: AgentsLike },
  engine: ChangeLedgerEngine,
  coordinator: TurnCheckpointCoordinator,
  sessionId: string,
  messageSeq: number,
  ...
  const { target, checkpoint } = await resolveMessageCheckpoint(ctx, engine, sessionId, messageSeq)
  if (checkpoint === undefined) {
    return { ...coordinator.state(sessionId, target.turn), fileRestoreAvailable: false }
  }
  ...
  if (inspection.changes.length === 0) return { ...common }
  if (detailsOnly || activeSessionIds.length > 0 || inspection.operationChanged) return { ...common }
  ...
  return { ...common, planId: plan.id, confirmation: plan.confirmation }
}

/** True only when the cwd is not inside any Git worktree, so file restore is unavailable by design. */

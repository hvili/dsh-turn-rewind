            ...
            if (request.method === 'POST') {
                ...
                if (action === 'rewindConversation') {
                    ...
                    if (!archived.result.ok) {
                        throw new ChangeLedgerError('CONVERSATION_REWIND_FAILED', `the conversation-only revision was created, but the previous revision could not be archived: ${archived.result.error.message}`);
                    }
...
async function previewResponse(ctx, engine, coordinator, sessionId, messageSeq, detailsOnly, offset, limit) {
    const { target, checkpoint } = await resolveMessageCheckpoint(ctx, engine, sessionId, messageSeq);
    if (checkpoint === undefined) {
